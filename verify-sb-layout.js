...
setTimeout(() => {
  try {
    ...
    assert('最优推荐标签位于利用率数字下方', (function(){
      var col=$('sbUtilCol'); if(!col) return false;
      var ch=Array.prototype.slice.call(col.children);
      return ch.indexOf($('sbUtilTag')) > ch.indexOf($('sbUtilBig'));
    })());
